...
echo $qq_ff;
blastn -task megablast -num_threads 24 -db $db_db/Rfer_all_proteins -outfmt '6 qseqid qstart qend qlen sseqid staxids sstart send bitscore nident evalue length' -query $ffn_dir/$qq_ff -out $out_dir/${qq_ff}.hits -evalue 0.001;
done
...
echo $qq_ff;
blastp -query $faa_dir/$qq_ff -num_threads 24 -db $db_db/cytochrome_protein -outfmt '6 qseqid qstart qend qlen sseqid staxids sstart send bitscore nident evalue length' -out $out_dir/${qq_ff}.hits -evalue 0.001;
done
